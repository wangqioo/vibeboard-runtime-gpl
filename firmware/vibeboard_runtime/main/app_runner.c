...
{
    BaseType_t created = xTaskCreatePinnedToCoreWithCaps(task,
                                                         name,
    ...
    ESP_LOGW(TAG, "PSRAM Lua task stack failed, retrying internal stack");
...
typedef struct {
    const vb_lua_app_context_t *app;
...
typedef struct {
    vb_lua_app_context_t app;
    vb_app_runner_result_t result;
} vb_lua_async_context_t;

static void entry_to_lua_context(const vb_app_registry_entry_t *entry, vb_lua_app_context_t *app)
{
    memset(app, 0, sizeof(*app));
    strlcpy(app->name, entry->name[0] ? entry->name : entry->id, sizeof(app->name));
    strlcpy(app->dir, entry->dir, sizeof(app->dir));
    strlcpy(app->path, entry->path, sizeof(app->path));
}

static esp_err_t registry_to_lua_context(const vb_app_registry_result_t *registry, vb_lua_app_context_t *app)
{
    if (registry == NULL || app == NULL || registry->app_count <= 0 || registry->first_app_path[0] == '\0') {
        return ESP_ERR_NOT_FOUND;
    }
    memset(app, 0, sizeof(*app));
    strlcpy(app->name, registry->first_app_name, sizeof(app->name));
    strlcpy(app->dir, registry->first_app_dir, sizeof(app->dir));
    strlcpy(app->path, registry->first_app_path, sizeof(app->path));
    return ESP_OK;
}

static esp_err_t run_lua_file(const vb_lua_app_context_t *app, vb_app_runner_result_t *result)
{
    ESP_LOGI(TAG, "Lua app start: %s", app->name);
    ...
    vb_lua_lvgl_set_app_dir(app->dir);
    vb_lua_lvgl_register(L);

    int lua_err = luaL_dofile(L, app->path);
    if (lua_err != LUA_OK) {
...
esp_err_t vb_app_runner_run(const vb_app_registry_result_t *app, vb_app_runner_result_t *result)
{
    ...
    vb_lua_app_context_t lua_app;
    err = registry_to_lua_context(app, &lua_app);
    if (err != ESP_OK) {
    ...
    BaseType_t created = create_lua_task(lua_task, "vb_lua", &context);
    if (created != pdPASS) {
...
esp_err_t vb_app_runner_run_entry(const vb_app_registry_entry_t *entry, vb_app_runner_result_t *result)
{
    ...
    vb_lua_app_context_t lua_app;
    entry_to_lua_context(entry, &lua_app);
    vb_lua_task_context_t context = {
        .app = &lua_app,
        .result = result,
        .status = ESP_FAIL,
                                                  ...
                                                  const vb_app_runner_launch_options_t *options)
{
    ...
    entry_to_lua_context(entry, &context->app);

    if (options != NULL && options->before_start != NULL) {
        options->before_start(options->user_data);
    }

    BaseType_t created = create_lua_task(lua_async_task, "vb_lua_launch", context);
    if (created != pdPASS) {
        fail_reserved_start(ESP_ERR_NO_MEM);
        free(context);
        return ESP_ERR_NO_MEM;
    ...
    ESP_LOGI(TAG, "Lua async launch: %s", context->app.name);
    return ESP_OK;
}
