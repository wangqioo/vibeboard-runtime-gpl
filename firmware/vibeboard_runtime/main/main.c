...
static void show_boot_screen(const vb_board_status_t *board, const vb_app_registry_result_t *apps, esp_err_t scan_err)
{
    lvgl_port_lock(0);
...
void app_main(void)
{
    ...
    show_boot_screen(&board, &apps, scan_err);
    ESP_LOGI(TAG, "VibeBoard Runtime ready: sd=%s apps=%d", board.sd_ok ? "ok" : "missing", apps.app_count);
}
