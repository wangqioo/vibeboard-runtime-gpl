...
- `module_host_api.*` now defines and initializes native host API groups for serial output, time/yield/delay, heap allocation, SD-rooted file open/read/seek/position/size/available/close, and task create/remove/yield/delay;
...
- extend host APIs with display and Lua transfer table after the linked adapter can consume the first host API group;
