...
The next NES implementation slice should create and own the linked `nes_core_bridge` runtime, not full emulation:

- add `module_host_api_v1 host_v1` and core-runtime state to `nes_native_adapter.*`, initialize it with `vb_nes_host_v1_shim_init(...)`, then call `nes_core_create(...)` from the linked adapter;
