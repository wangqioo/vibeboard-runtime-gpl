...
Update 2026-07-03: Camera/Photos and local Voice AI moved into the current board baseline. `apps/camera` now provides Runtime GC2145 preview, serialized capture, BMP saves under `/sd/data/camera/photos`, `/app/api` / `/app/photos` / `/app/capture`, native busy/photo overlays, and an on-device gallery. The user verified that capture no longer exits to Launcher and that both the Camera gallery and standalone `apps/photos` can display captured photos. Audio/Voice AI also has first physical closure: the speaker tone is audible, file-backed recording/playback works after channel/rate tuning, and `apps/voice_ai` successfully recorded speech, uploaded PCM to the desktop bridge, used local Apple Speech STT, and displayed the returned recognition result. Remaining work is regression/soak/productization: cloud STT/LLM provider, privacy/logging policy, long audio soak, camera capture/delete soak, broader visual QA, real BLE/Xbox, and performance parity.

Update 2026-07-06: SD resource-read performance now has a board-measured baseline. The LCKFB board is SDMMC 1-bit on GPIO47/48/21. Runtime file serving now uses a dynamically allocated internal-DMA buffer with 4KB default chunks and a fallback down to 512B; fixed 16KB failed on board due to contiguous internal DMA pressure. `/debug/sd-bench` shows Weather `storm.bmp` raw SD reads improved from about 151-155 ms at 20 MHz to about 119-124 ms at 40 MHz high-speed mode, roughly 1.2 MB/s. End-to-end `/apps/file` downloads of the same 153666-byte BMP still show HTTP/WiFi variance around 1.13-1.34 s, so the next performance work should split SD read, decode/convert, LVGL lock, and HTTP transfer instead of treating SD as the only bottleneck.

## Current Baseline

```text
local main with Weather/Web UI/deployment productization, Weather 320x240 BMP canvas background metrics, SD raw-read benchmark endpoint plus 40MHz SDMMC high-speed board evidence, Lua app manager lifecycle APIs, BOOT-to-Lua HOME forwarding, HTTP key/gamepad smoke hooks, smoke_key repeat metrics, Camera preview/capture/gallery/Photos board evidence, Voice AI local STT/reply display evidence, I2S RX nonzero PCM plus audible sustained TX-only 440Hz tone-write smoke, NES host-audio backend board smoke, NES legal-ROM frame/audio smoke, nesgame selector/gamepad injection board evidence, smoke_nes native host gamepad synthetic board evidence, BTC minimal network price-panel board smoke through a local mock, Settings minimal key-driven panel board smoke, hwmon minimal host metrics board smoke through a local mock, Spectrum real-audio visualizer board smoke, videos GIF playlist board smoke, photos PNG image playlist and captured-photo display evidence, plane sprite game board smoke, lv-benchmark minimal runtime benchmark board smoke, and Codex Buddy bridge/GIF/HOME permission board smoke; broad physical visual QA, real BLE/Xbox, cloud provider/network checks, HTTP/WiFi resource transfer optimization, and long soak remain
```
...
- Runtime-owned config writes are implemented and build-verified:
  ...
  - I2S TX driver-write smoke is board-verified with `i2s.write`, `dout_pin`, `writes`, and `tx_bytes`; 2026-06-24 added sustained low-DMA TX-only 440Hz tone metrics with `tone_writes=18,tx_bytes=4608`. The user later confirmed the 440 Hz tone was audible through the physical speaker; Voice AI playback quality and long audio soak remain pending.
