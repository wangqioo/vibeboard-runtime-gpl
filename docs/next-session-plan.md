...
- Do not claim Lua `app.launch(...)` board completion until `apps/smoke_app_manager` has been uploaded, its software HOME short event or a physical HOME short press has handed off to `smoke_key`, and that transition has been observed on the ESP32-S3 board.
