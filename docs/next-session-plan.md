...
- after manifest validation, the Runtime now routes native modules through `native_module_static_adapter.*`, initializes the host API table, calls the static `vb_native_module_init(&host_api, &module)` adapter seam, and maps unsupported init results to precise host API errors before a real ELF loader or emulator core is wired in;
