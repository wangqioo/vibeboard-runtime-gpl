...
| Native modules | `require("nes")` through the runtime native module searcher, `native_abi_version = vibeboard-native-module-abi@1` | `build-verified first slice` | Runtime now exposes a native ABI version, installs a Lua native module searcher before app entry execution, and routes `apps/nesgame` through `local nes = require("nes")`. This slice intentionally stops at precise loader errors such as `Native module load failed`, `Native module symbol missing`, `Native module ABI mismatch`, and `Native module host API unsupported`; the NES emulator core, display host API, audio, and gamepad-native bridge are still pending. |
