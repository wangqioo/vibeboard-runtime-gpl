...
| Native modules | `require("nes")` through the runtime native module searcher, `native_abi_version = vibeboard-native-module-abi@1` | `build-verified first slices` | Runtime now exposes a native ABI version, installs a Lua native module searcher before app entry execution, validates app-local `native/nes.vbn` descriptors, routes validated modules through a static adapter facade, and initializes host API groups for serial, time, heap, SD file access, and task create/remove/yield/delay. `apps/smoke_nes` isolates this loader/stub path from the full NES game UI and is included in demo packaging. `nes.start(...)` still returns `false, "native executor pending"`; the NES emulator core, display host API, audio, and gamepad-native bridge are still pending. |
