...
| Staged app upload | `POST /stage?app=<id>&path=<relative>`, `POST /commit?app=<id>`, `POST /discard?app=<id>` | `build-verified` | Static firmware guardrails verify staged endpoint registration, staging root `/sdcard/.staging`, staged `app.info` plus entry validation, running-app `409 Conflict`, rename-based replacement, and registry rescan. |
...
| Mac staged uploader default | `upload:app` default staged mode, `--mode direct` compatibility mode | `build-verified` | `npm run test:uploader` verifies default `/discard -> /stage -> /commit -> /apps`, direct `/install -> /rescan -> /apps`, retry behavior, and best-effort staged discard after failure. |
