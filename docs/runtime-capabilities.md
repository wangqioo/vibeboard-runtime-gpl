...
| App lifecycle status | `GET /status` field `state` with `idle`, `starting`, `running`, `stopping`, `failed`; compatibility fields `running` and `current_app` remain | `build-verified` | Phase 5B adds the explicit app runner lifecycle state. `running` remains true for `starting`, `running`, and `stopping`. After normal completion the state returns to `idle`; after Lua/task failure it becomes `failed`. `current_app` is cleared once the task exits, including the `failed` state, with the failure detail kept in firmware logs for now. |
