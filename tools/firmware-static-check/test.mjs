...
describe("vibeboard runtime firmware static guardrails", () => {
  ...
  it("registers a Lua camera module backed by the board GC2145 camera", () => {
    ...
    assert.match(boardSource, /s_camera_overlay_has_photo/);
    ...
    assert.match(boardHeader, /vb_board_camera_overlay_photo_set/);
    assert.match(boardSource, /vb_board_camera_overlay_photo_set/);
    assert.match(functionBody(boardSource, "vb_board_camera_overlay_photo_set"), /s_camera_overlay_enabled/);
    ...
    assert.match(boardSource, /s_camera_overlay_has_photo\s*\?\s*0x7dd3\s*:\s*0x39e7/);
    ...
    assert.match(cameraSource, /camera_overlay_photo/);
    assert.match(cameraSource, /"overlay_photo",\s*camera_overlay_photo/);
  ...
  it("ships Camera as a real Runtime capture app", () => {
    ...
    assert.match(luaFunctionBody(source, "start_preview"), /sync_overlay_photo\(\)/);
    ...
    assert.match(source, /local function sync_overlay_photo\(\)/);
    assert.match(luaFunctionBody(source, "sync_overlay_photo"), /if not APP\.preview then\s+return\s+end/);
    assert.match(luaFunctionBody(source, "sync_overlay_photo"), /camera\.overlay_photo\(#APP\.photos > 0\)/);
    assert.match(luaFunctionBody(source, "list_photos"), /sync_overlay_photo\(\)/);
    assert.match(source, /remember_photo\(filename,\s*APP\.last_bytes\)/);
    assert.match(luaFunctionBody(source, "remember_photo"), /sync_overlay_photo\(\)/);
    assert.match(source, /forget_photo\(name\)/);
    assert.match(luaFunctionBody(source, "forget_photo"), /sync_overlay_photo\(\)/);
