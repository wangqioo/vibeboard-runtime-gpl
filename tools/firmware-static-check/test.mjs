...
describe("vibeboard runtime firmware static guardrails", () => {
  ...
  it("registers a Lua camera module backed by the board GC2145 camera", () => {
    ...
    assert.match(sdkconfigDefaults, /CONFIG_CAMERA_PSRAM_DMA=y/);
    ...
    assert.match(boardHeader, /VB_CAMERA_XCLK_FREQ_HZ\s+24000000/);
    ...
    assert.match(boardSource, /vb_board_draw_rgb565\(0,\s*0,\s*frame->width,\s*frame->height/);
    ...
    assert.match(smokeSource, /width\s*=\s*160/);
    assert.match(smokeSource, /height\s*=\s*120/);
